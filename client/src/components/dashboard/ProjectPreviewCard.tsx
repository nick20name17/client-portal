...
function injectHideScript(html: string): string {
  const hideScript = `<script>(function(){var s=document.createElement("style");s.textContent="*{animation:none!important;transition:none!important}";document.head.appendChild(s);var isPreview=window.self!==window.parent;if(isPreview){var reveals=document.querySelectorAll(".reveal, .fade-in, .slide-in, .animate-in, .animate, [data-animate], [data-reveal]");for(var i=0;i<reveals.length;i++){reveals[i].style.opacity="1";reveals[i].style.visibility="visible"}}function hideFixed(){var all=document.querySelectorAll("*");for(var i=0;i<all.length;i++){var el=all[i];if(getComputedStyle(el).position==="fixed"||getComputedStyle(el).position==="sticky"){el.style.display="none"}}}function settle(){hideFixed();new MutationObserver(function(mutations){for(var m=0;m<mutations.length;m++){var added=mutations[m].addedNodes;for(var n=0;n<added.length;n++){var node=added[n];if(node.nodeType===1){var pos=getComputedStyle(node).position;if(pos==="fixed"||pos==="sticky"){node.style.display="none"}}}}}).observe(document.body,{childList:true})}var timer=setTimeout(settle,1500);window.addEventListener("message",function(e){if(e.data&&e.data.type==="PREVIEW_SETTLE_QUERY"){clearTimeout(timer);settle();window.parent.postMessage({type:"PREVIEW_SETTLED"},"*")}});})()</script>`;
