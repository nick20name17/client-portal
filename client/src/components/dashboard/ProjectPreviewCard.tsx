...
function injectHideScript(html: string): string {
  const hideScript = `<script>(function(){var s=document.createElement("style");s.textContent="*{animation:none!important;transition:none!important!important}";document.head.appendChild(s);var isPreview=window.self!==window.parent;if(isPreview){var raf=window.requestAnimationFrame;window.requestAnimationFrame=function(cb){return raf(function(){cb();cb();cb()})};document.querySelectorAll("*").forEach(function(el){el.style.transitionDuration="0s";el.style.animationDuration="0s"});document.querySelectorAll("[style*='opacity: 0'], [style*='display: none'], .reveal, .hidden").forEach(function(el){el.style.opacity="1";el.style.visibility="visible";el.style.display="";el.classList.remove("is-hidden");if(el.classList){Array.from(el.classList).forEach(function(c){if(c.includes("hidden")||c.includes("collapse")||c==="invisible")el.classList.remove(c)})}})}function hide(){document.querySelectorAll("*").forEach(function(el){var p=getComputedStyle(el).position;if(p==="fixed"||p==="sticky")el.style.setProperty("display","none","important")})};function doHide(){hide();new MutationObserver(hide).observe(document.body,{childList:true,subtree:true})};var readyTimeout=setTimeout(function(){doHide()},2000);window.addEventListener("message",function(e){if(e.data&&e.data.type==="PREVIEW_SETTLE_QUERY"){clearTimeout(readyTimeout);doHide();window.parent.postMessage({type:"PREVIEW_SETTLED"},"*")}});})()</script>`;
